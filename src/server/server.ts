...
init().then(() => {
  ...
  httpserver.listen(config.http.http_port, (err) => {
    ...
    // Start WSS instance
    WSSHandler();
    console.info(`HttpServer started on ${config.http.http_port}`);
  });
});
