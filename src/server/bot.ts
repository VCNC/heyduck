...
const handleBurritos = async (
  ...
  if (!enableDecrement) {
    ...
    if (updates.length > diff) {
      notifyUser(
        giver,
        `${updates.length} 개의 :duck:을 주는데 실패했습니다. 오늘 줄 수 :duck:의 개수는 ${diff}개 입니다.`,
      );
      return false;
    }
    ...
    if (incUpdates.length) {
      if (incUpdates.length > diffInc) {
        notifyUser(
          giver,
          `${updates.length} 개의 :duck:을 주는데 실패했습니다. 오늘 줄 수 :duck:의 개수는 ${diffInc}개 입니다.`,
        );
      } else {
  ...
  notifyUser(
    giver,
    `${receivers.map((it) => `<@${it}>`).join(' ')}에게 ${
      updates.length
    }개의 :duck:을 주었습니다. 오늘 ${leftOverDucks}개의 :duck:을 더 줄 수 있습니다.`,
  );
  receivers.forEach((receiver) => {
    notifyUser(
      receiver,
      `<@${giver}>님이 <#${channel}>에서 1개의 :duck:을 주었습니다.`,
    );
  });
  return true;
};
